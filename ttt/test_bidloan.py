...
class BidLoanTestCase(unittest.TestCase):
    ...
    def test_bid_loan(self, case):
        ...
        if "#lpwd#" in case.request_data:
            # 将登录密码从配置文件中读取并替换掉用例中的#login_phone#
            case.request_data = case.request_data.replace("#lpwd#", conf.get("test_data", "pwd"))

        # 用配置文件的mobile_phone对应的memberid替换用例的#memberid#
        if "#memberid#" in case.request_data:
            mobile_phone = conf.get("test_data", "mobile_phone")
            memberid = self.db.find_one("SELECT Id FROM member WHERE MobilePhone={}".format(mobile_phone))[0]
            case.request_data = case.request_data.replace("#memberid#", str(memberid))

        # 用配置文件的mobile_phone对应的pwd替换用例的#pwd#
        if "#pwd#" in case.request_data:
            case.request_data = case.request_data.replace("#pwd#", conf.get("test_data", "pwd"))

        # 用查库的loanid替换用例的#loanid#
        if "#loanid#" in case.request_data:
            loanid = self.db.find_one("SELECT Id FROM loan WHERE Status=4 AND Amount>50000 GROUP BY CreateTime DESC LIMIT 1")[0]
            case.request_data = case.request_data.replace("#loanid#", str(loanid))

        # 用查库的loanid替换用例的#loanid#
        if "#floanid#" in case.request_data:
            loanid = self.db.find_one("SELECT Id FROM loan WHERE Status!=4 AND Amount>50000 GROUP BY CreateTime DESC LIMIT 1")[0]
            case.request_data = case.request_data.replace("#floanid#", str(loanid))
        ...
        if "#mloanid#" in case.request_data:
            loanid = self.db.find_one("SELECT Id FROM loan WHERE `Status`=5 GROUP BY CreateTime DESC LIMIT 1")[0]
            case.request_data = case.request_data.replace("#mloanid#", str(loanid))

        if "#lmemberid#" in case.request_data:
            memberid = self.db.find_one("SELECT Id FROM member WHERE LeaveAmount<{}".format(eval(case.request_data)["amount"]))[0]
            case.request_data = case.request_data.replace("#lmemberid#", str(memberid))
            case.request_data = case.request_data.replace("#lpwd#", conf.get("test_data", "lpwd"))
        ...
        if case.check_mysql:
            ...
            mobile_phone = conf.get("test_data", "mobile_phone")
            memberid = self.db.find_one("SELECT Id FROM member WHERE MobilePhone={}".format(mobile_phone))[0]
            case.check_mysql = case.check_mysql.replace("#memberid#", str(memberid))
